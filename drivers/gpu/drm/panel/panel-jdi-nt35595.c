...
{
	return container_of(panel, struct jdi_nt35595, panel);
}
...
static int jdi_nt35595_on(struct jdi_nt35595 *ctx)
{
	...
	dsi_dcs_write_seq(dsi, 0x13, 0x01);
	usleep_range(1000, 2000);
	...
	usleep_range(1000, 2000);
	dsi_dcs_write_seq(dsi, 0x53, 0x02);
	usleep_range(1000, 2000);
	...
	usleep_range(1000, 2000);
	dsi_dcs_write_seq(dsi, 0x3b, 0x03, 0x04, 0x04, 0x10, 0x10);
	usleep_range(1000, 2000);
	...
	usleep_range(1000, 2000);
	dsi_dcs_write_seq(dsi, 0x55, 0x81);
	usleep_range(1000, 2000);
				 ...
				 struct drm_connector *connector)
{
	...
	mode = drm_mode_duplicate(connector->dev, &jdi_nt35595_mode);
	if (!mode)
		return -ENOMEM;
...
static int jdi_nt35595_probe(struct mipi_dsi_device *dsi)
{
	...
	drm_panel_init(&ctx->panel, dev, &jdi_nt35595_panel_funcs,
...
static int jdi_nt35595_remove(struct mipi_dsi_device *dsi)
{
